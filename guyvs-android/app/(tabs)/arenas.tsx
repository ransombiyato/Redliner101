...
export default function ArenasScreen() {
  ...
  return <AppScreen><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <Text style={styles.title}>Arena Creator</Text><Text style={styles.subtitle}>Draw rings, place shapes, tune bounce, and stack local physics zones.</Text>
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>{state.project.arenas.map((item) => <ActionButton key={item.id} label={item.name} tone={item.id === selectedId ? "magenta" : "ghost"} small onPress={() => setSelectedId(item.id)} />)}<ActionButton label="+" tone="cyan" small onPress={() => { const created = makeArena(); addArena(created); setSelectedId(created.id); }} /></ScrollView>
    {arena ? <>
...
const styles = StyleSheet.create({ content: { padding: 18, gap: 16, paddingBottom: 32 }, title: { color: palette.text, fontSize: 30, fontWeight: "900", letterSpacing: -0.8 }, subtitle: { color: palette.muted, marginTop: -8, fontSize: 14, lineHeight: 20 }, chips: { gap: 8, paddingVertical: 2 }, row: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: palette.panelRaised, borderRadius: 12, padding: 10 }, flex: { flex: 1, gap: 2 }, shapeIcon: { width: 23, height: 23, borderWidth: 3, borderRadius: 6 }, zoneIcon: { width: 23, height: 23, borderRadius: 7 }, itemTitle: { color: palette.text, fontSize: 13, fontWeight: "800" }, itemMeta: { color: palette.muted, fontSize: 11 }, presetRow: { flexDirection: "row", gap: 8, flexWrap: "wrap" }, drawCanvas: { height: 210, borderRadius: 14, backgroundColor: palette.ink, borderWidth: 1, borderColor: palette.border, overflow: "hidden", position: "relative" }, point: { position: "absolute", width: 10, height: 10, borderRadius: 5, backgroundColor: palette.lime }, canvasHint: { color: palette.muted, fontSize: 12, alignSelf: "center", marginTop: 95 }, });
